    ...
    def get_ocr_text_for_hash(self, content_hash: str) -> list[dict] | None:
        ...
        try:
            cursor = self._conn.execute(
                'SELECT content_hash, ocr_text, ocr_engine, ocr_preprocess, '
                'ocr_lang, ocr_dpi, ocr_psm, ocr_oem, ocr_rotation, ocr_at, '
                'pdf_size, pdf_mtime, actual_engine '
                'FROM ocr_cache WHERE content_hash LIKE ?',
                (f'{content_hash}%',),
            )
            for r in cursor.fetchall():
                rows.append(_ocr_row_from_db(r).to_dict())
...
def _ocr_row_from_db(row: tuple) -> OCRRow:
    """Build an OCRRow from a sqlite3 ocr_cache row tuple (in canonical order)."""
    # The SELECT order in _build_indices:
    #   content_hash, ocr_text, ocr_engine, ocr_preprocess, ocr_lang,
    #   ocr_dpi, ocr_psm, ocr_oem, ocr_rotation, ocr_at, pdf_size,
    #   pdf_mtime, actual_engine
    text = row[1] or ''
    header, _body = parse_ocr_header(text)
    return OCRRow(
        content_hash=row[0] or '',
        ocr_text=text,
        ocr_engine=row[2] or 'tesseract',
        ocr_preprocess=row[3] or 'none',
        ocr_lang=row[4] or 'chi_sim+eng',
        ocr_dpi=row[5],
        ocr_psm=row[6],
        ocr_oem=row[7],
        ocr_rotation=row[8],
