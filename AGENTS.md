...
- **`setup.sh` is broken three ways** — creates `venv/` (should be `myenv/`), runs `pip install python-docs` (typo, not a real package), then `pip freeze > requirements.txt` would clobber the checked-in list. Use the manual flow instead:
  ...
  ```
- **`requirements.txt` is incomplete.** It lists `lxml`, `pillow`, `python-docx`, `typing_extensions` only. `file-share` and `llm-chat` `import aiohttp`, so installing from `requirements.txt` alone will leave those two tools broken. Install `aiohttp` separately, or regenerate requirements after a working `myenv/` is set up.
