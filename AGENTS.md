...
- **NumPy must stay < 2.0.** `paddlepaddle 2.6.2` and `paddleocr 2.7.3` ship C extensions compiled against NumPy 1.x ABI. NumPy 2.0 changed the C ABI (now `0x2000000` instead of `0x1000009`) and these extensions refuse to load. Symptoms: `ImportError: numpy.core.multiarray failed to import` or `RuntimeError: module compiled against ABI version 0x1000009 but this version of numpy is 0x2000000`. `requirements.txt` pins `numpy<2.0` to prevent this. If you hit it, `pip install 'numpy<2.0'` then `pip install --force-reinstall --no-deps paddlepaddle==2.6.2 paddleocr==2.7.3`.
